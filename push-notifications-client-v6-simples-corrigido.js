// ========== PUSH NOTIFICATIONS SIMPLES v6 - CORRIGIDO ==========
// Versão reduzida sem dependencies issues
console.log('[Push] Iniciando v6-simples corrigido...');
...
try {
    ...
    const poll = async () => {
      try {
        ...
        for (const delivery of deliveries) {
          ...
          // Substituir variáveis (COM PROTEÇÃO contra AppState undefined)
          let body = msg.body;
          let userName = 'Usuário';
          try {
            if (typeof AppState !== 'undefined' && AppState?.userProfile?.full_name) {
              userName = AppState.userProfile.full_name;
            } else if (session.user.user_metadata?.full_name) {
              userName = session.user.user_metadata.full_name;
            }
          } catch (e) {
            console.warn('[Push] Erro ao obter nome do usuário:', e);
          }
        ...
        console.error('[Push] Erro no polling:', e.message || e);
      }
    };
  ...
  console.log('[Push] v6-simples CORRIGIDO carregado ✅');
