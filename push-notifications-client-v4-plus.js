// ========== PUSH NOTIFICATIONS - FENDA MUSIC CLIENT (v4+) ==========
// Compatível com sw.js v16 existente
// Carregue APÓS supabase-config.js e ANTES de player-core.js
// NO PLAYER.HTML: <script src="push-notifications-client-v4-plus.js"></script>
...
// Evita duplicatas quando página recarrega
...
function markNotificationAsDisplayed(messageId) {
  ...
  if (!displayed.includes(messageId)) {
    ...
    // Manter apenas últimas 100 para não encher localStorage
...
async function setupPushNotifications() {
  ...
  try {
    // SW já deve estar registrado, mas garantir
...
function isNotificationPanelOpen() {
  // Verifica se elemento com ID 'notifList' existe e é visível
...
function refreshNotificationPanel() {
  try {
    ...
    const notifList = document.getElementById('notifList');
    if (!notifList) return; // Painel não está no DOM
    
    // Se painel está aberto, chamar openNotifications para rerenderizar
...
// ========== POLLING DE NOTIFICAÇÕES (Fallback robusto) ==========
...
async function startNotificationPolling() {
  ...
  const poll = async () => {
    try {
      const { data: { session } } = await supabaseClient.auth.getSession();
      if (!session) {
        console.warn('[Push] Não autenticado, abortando poll');
        return;
      }
      ...
      for (const delivery of deliveries) {
        ...
        // ========== DEDUPLICAÇÃO ==========
        // Se já foi exibida nesta sessão, pular
        ...
        // Obter dados do usuário (do localStorage ou do estado global)
        ...
        // ========== ADICIONAR AO PAINEL DE NOTIFICAÇÕES DO APP ==========
        try {
  ...
  // Depois repetir a cada POLLING_INTERVAL
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.addEventListener('message', event => {
    ...
    if (type === 'NOTIFICATION_CLICK') {
      ...
      // Se tem musicId, tentar tocar a música
...
window.addEventListener('load', async () => {
  ...
  setTimeout(async () => {
    try {
      // Tentar setup de push real (web push + VAPID)
...
console.log('[Push] Módulo v4+ carregado ✅');
